...
<template>
    <div ref="base" class="y relative items-center text-ui-200 min-h-screen grow overflow-x-clip" :class="getGame() === 'soundodger' ? 'theme-soundodger' : ''">
        ...
        <div class="y relative grow items-center w-full">
            <div v-if="isAuthenticated() && !isPremium() && $page.props.app.patron" class="flex flex-wrap py-1 justify-center items-center text-center w-full px-8 text-sm" :class="fullwidth ? 'bg-ui-950' : 'mt-2 bg-ui-900 rounded-lg border border-ui-700 lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl'">
                <span>Hyperbolus is supported by</span>
                <div class="x items-center space-x-2 pl-1.5 pr-1">
            ...
            <AdUnit v-else-if="!isAuthenticated() && !fullwidth && !noAds" :cta="false" type="banner" slot-key="2978602886" ad-style="height:90px;min-width:400px;width:100%;" class="mt-2 w-full lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl px-8"/>
            ...
            <div v-if="decorations" class="x justify-center w-full my-2">
                <AdUnit v-if="!isPremium() && !noAds" type="skyscraper" slot-key="2215741975" class="hidden 2xl:block sticky top-2"/>
                ...
                <AdUnit v-if="!isPremium() && !noAds" type="skyscraper" slot-key="9902660301"  class="hidden 2xl:block sticky top-2"/>
            </div>
            <slot v-else/>
            <AdUnit v-if="false && !isPremium() && !noAds" :cta="true" type="banner" slot-key="4229314886" ad-style="min-width:400px;width:100%;" class="mb-2 w-full lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl px-8"/>
        </div>
        <Footer/>
        <CookieConsent/>
    </div>
