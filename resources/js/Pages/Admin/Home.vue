...
const calcReputations = useForm({
    action: 'recalculate reputations'
})

const updateMacros = useForm({
...
<template>
    <admin-layout title="Home">
        ...
        <div class="x items-center space-x-1">
            ...
            <Button @click="calcReputations.post(route('system.home'))" class="w-fit" :class="{ 'opacity-25': calcReputations.processing }" :disabled="calcReputations.processing">{{ calcReputations.processing ? 'Working...' : 'Recalculate Reputations' }}</Button>
            <div class="x ml-2 text-green-500 opacity-0 transition-all" :class="{'!opacity-100': !calcReputations.isDirty && calcReputations.recentlySuccessful}">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5 mr-0.5">
                    <path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd" />
                </svg>
                <span>Done!</span>
            </div>
            <Button @click="updateMacros.post(route('system.home'))" class="w-fit" :class="{ 'opacity-25': updateMacros.processing }" :disabled="updateMacros.processing">{{ updateMacros.processing ? 'Working...' : 'Update Macros' }}</Button>
            <div class="x ml-2 text-green-500 opacity-0 transition-all" :class="{'!opacity-100': !updateMacros.isDirty && updateMacros.recentlySuccessful}">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5 mr-0.5">
                    <path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd" />
                </svg>
                <span>Done!</span>
