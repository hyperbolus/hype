...
const {searchQuery, searchResults, vote, verify} = useTagManager(props.tags, toRef(props, 'level'), toRef(props, 'levelTagVotes'), 'tags');

// TODO: does tag manager need to be disposed on unmount?
</script>
<template>
    <Layout>
        <div class="x !mt-0 flex-col md:flex-row bg-ui-950 border border-ui-900 rounded-lg px-4 py-2 space-x-2">
            <div class="y space-y-2 md:w-1/3">
                ...
                <div v-if="isAuthenticated()" class="y pane space-y-2">
                    <div>
                        <span class="block font-medium text-sm text-ui-300">Tag</span>
                        <Input @click="dropdown.open = true" v-model="searchQuery" class="mt-1" type="text" placeholder="Tag"/>
                    ...
                    <Errors/>
                    <Button @click="vote(searchQuery)" class="w-fit">Submit Tag</Button>
                </div>
                <p v-else class="pane">You must be <Link :href="route('auth::login')" class="underline">logged in</Link> to add or vote on tags.</p>
                <p class="pane">Here you can vote on which tags do or don't fit a level by voting for or against them.</p>
                ...
                <div class="y space-y-2">
                    ...
                    <div v-for="tag in level.tags" class="x pane justify-between">
                        <div class="x items-center space-x-2">
                            <svg @click="isAdmin() ? verify(tag.id, !tag.pivot.verified) : void(0)" v-if="tag.pivot.verified || isAdmin()" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="inline rounded-full w-5 h-5" :class="{'text-blue-500': tag.pivot.verified, 'cursor-pointer': isAdmin(), 'opacity-50': !tag.pivot.verified}">
