    ...
    <app-layout title="Home">
        ...
        <div class="x h-64 justify-center w-full bg-neutral-500 bg-cover bg-center" style="box-shadow: rgba(0, 0, 0, 0.85) 0 -100px 75px -25px inset;" :style="'background-image: url(\'' + (level.banner_url ?? background) + '\')'">
            <div class="y justify-end w-full lg:max-w-5xl xl:max-w-6xl py-2">
                <div class="x justify-between items-end">
                    ...
                    <Link v-if="$page.props.auth && $page.props.user.roles.includes('admin')" :href="route('levels.edit', level)" class="px-2 py-1 rounded bg-neutral-100 dark:bg-neutral-800">Edit</Link>
                </div>
            </div>
        </div>
        <div class="x justify-center bg-neutral-200 dark:bg-neutral-900 w-full">
            <div class="x items-center space-x-2 lg:max-w-5xl xl:max-w-6xl w-full px-2 py-2">
                ...
                <Link v-for="tag in level.tags" :href="route('tags.show', tag)" class="x items-center text-neutral-700 dark:text-neutral-300 px-2 py-1 text-sm rounded  bg-neutral-100 dark:bg-neutral-800 capitalize">
                ...
                <Link :href="route('levels.tags.show', level)" class="text-sm rounded hover:underline cursor-pointer">More...</Link>
            </div>
        </div>
        <div class="x max-w-6xl w-full gap-4 py-4">
            <div class="y space-y-4 w-3/4">
                <form v-if="$page.props.auth" @submit.prevent="submit" class="x box gap-4">
                    <div class="space-y-2 w-full">
                        <h2 class="text-xl">{{ props.review ? 'Edit Your' : 'Submit' }} Rating</h2>
                        <textarea v-model="form.body" class="resize-none resize-y w-full rounded bg-neutral-100 dark:bg-neutral-900 border-neutral-300 dark:border-neutral-700"></textarea>
            ...
            <div class="y space-y-4 w-1/4">
                ...
                <div v-if="level.videos.length === 0" class="box">
                    No Videos :(
                </div>
                <div v-else v-for="video in level.videos" class="box">
