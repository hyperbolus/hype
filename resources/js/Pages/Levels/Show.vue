    ...
    <app-layout title="Home">
        ...
        <div class="flex max-w-6xl w-full gap-4 py-4">
            <div class="flex flex-col space-y-4 w-3/4">
                ...
                <form v-if="$page.props.auth" @submit.prevent="submit" class="flex flex-col rounded px-2 py-2 space-y-2 bg-gray-50 border border-gray-300">
                    ...
                    <span @click="submit" class="w-fit rounded px-2 py-1 bg-gray-200 hover:bg-gray-300 cursor-pointer">Submit Rating</span>
                </form>
                <div v-else class="flex flex-col rounded px-2 py-2 space-y-2 bg-gray-50 border border-gray-300">
                    <h2 class="text-xl">Log in to submit a review</h2>
                </div>
                <h2 class="font-bold text-2xl">Reviews</h2>
                <div v-for="review in level.reviews" class="rounded px-2 py-2 space-y-2 bg-gray-50 border border-gray-300">
            ...
            <div class="flex flex-col space-y-4 w-1/4">
                ...
                <div v-if="level.videos.length === 0" class="rounded bg-neutral-50 px-4 py-2 border border-gray-300">
                    No Videos :(
                </div>
                <div v-else v-for="video in level.videos" class="rounded bg-neutral-50 px-4 py-2 border border-gray-300">
