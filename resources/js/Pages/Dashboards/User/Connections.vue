...
<template>
    <dashboard-layout>
        <div class="y space-y-2">
            <div v-for="p in platforms" class="flex justify-between space-x-2 items-center px-4 py-3 bg-ui-900 rounded-md">
                <div class="flex space-x-4 items-center">
                    <svg v-if="p.key === 'twitter'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 209" class="size-10">
                    ...
                    <svg v-if="p.key === 'discord'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 199" class="size-10">
                    ...
                    <svg v-if="p.key === 'patreon'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 247" class="size-10">
                        <path fill="#FF424D" d="M45.136 0v246.35H0V0zm118.521 0C214.657 0 256 41.343 256 92.343s-41.343 92.343-92.343 92.343s-92.343-41.344-92.343-92.343S112.658 0 163.657 0"/>
                    </svg>
                    <span>{{ p.name }}</span>
                    <span v-if="accounts[p.id]" class="px-2 py-0.5 text-sm bg-ui-800 rounded-md">{{ accounts[p.id].username }}</span>
                </div>
                <a v-if="accounts[p.id]" :href="route('auth::oauth', p.key) + '?unlink'" class="rounded-md px-4 py-1 group bg-green-500 hover:bg-red-500 text-white"><span class="group-hover:hidden">Connected</span><span class="hidden group-hover:inline">Unlink</span></a>
                <a v-else :href="route('auth::oauth', p.key)" class="rounded-md px-4 py-1 bg-ui-700">Link</a>
            </div>
        </div>
    </dashboard-layout>
</template>
