<template>
    <app-layout>
        <template #breadcrumbs> / <Link href="/settings" class="underline">settings</Link> / <Link href="/settings/2fa" class="underline">2fa</Link></template>
        ...
        <div v-if="twoFactorEnabled">
            <div v-if="qrCode">
                ...
                <div class="flex flex-row justify-center mt-4" v-html="qrCode">
                </div>
            </div>
        ...
        <p class="underline mt-2 text-center"><Link href="/settings">Go Back</Link></p>
    </app-layout>
</template>
...
export default defineComponent({
    ...
    methods: {
        ...
        showQrCode() {
            return axios.get(route('auth::2fa.qr'))
                .then(response => {
        ...
        showRecoveryCodes() {
            return axios.get(route('auth::2fa.recovery'))
                .then(response => {
        ...
        regenerateRecoveryCodes() {
            axios.post(route('auth::2fa.recovery'))
                .then(response => {
                    this.showRecoveryCodes()
                })
        },
